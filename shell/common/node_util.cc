...
v8::MaybeLocal<v8::Value> CompileAndCall(
  ...
  if (!builtin_loader->Get()) {
    ...
    if (std::getenv("ELECTRON_INJECT_BAD_JS2C_CODE_CACHE")) {
      // Test hook: feed a corrupt cache to verify V8 rejects it cleanly.
      static constexpr uint8_t kGarbage[64] = {0xAB};
      std::vector<node::builtins::CodeCacheInfo> bad;
      bad.push_back(
          {"electron/js2c/sandbox_bundle",
           node::builtins::BuiltinCodeCacheData(kGarbage, sizeof(kGarbage))});
      loader->RefreshCodeCache(bad);
    } else if (!cache.empty()) {
