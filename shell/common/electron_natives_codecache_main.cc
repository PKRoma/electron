...
// Only the bundles each flavor's processes actually compile. Keep `params`
// in sync with the CompileAndCall call sites (the spec catches drift).
std::vector<Bundle> BundlesForFlavor(std::string_view flavor) {
  const Bundle kSandbox{"electron/js2c/sandbox_bundle", true, {"binding"}};
  const Bundle kIsolated{
      "electron/js2c/isolated_bundle", true, {"isolatedApi"}};
  const Bundle kPreloadRealm{
      "electron/js2c/preload_realm_bundle", true, {"binding"}};
  const Bundle kNodeInit{
      "electron/js2c/node_init", true, {"process", "require"}};
  const Bundle kBrowserInit{"electron/js2c/browser_init", false, {}};
  const Bundle kRendererInit{"electron/js2c/renderer_init", false, {}};
  const Bundle kUtilityInit{"electron/js2c/utility_init", false, {}};
  const Bundle kWorkerInit{"electron/js2c/worker_init", false, {}};
